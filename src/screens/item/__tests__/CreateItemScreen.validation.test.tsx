...
/**
 * One test per file (see the same note on `CreateItemScreen.save.test.tsx`):
 * a `fireEvent.press` that runs through `react-hook-form`'s async zod
 * resolver leaves pending, un-`act()`-wrapped form-state updates that
 * corrupt anything rendered afterwards — a second `render()` call in the
 * same file, *and* a second `fireEvent.press` in the same test, both
 * reproduced it (confirmed by isolating each). One resolver-triggered submit
 * per file/test avoids it; `CreateItemScreen.price.test.tsx` covers the
 * numeric-price rejection separately.
 */
describe('CreateItemScreen validation', () => {
  beforeEach(() => {
